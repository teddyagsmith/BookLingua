...
export function renderCustomerPackageEmail(input:{authorName:string;bookTitle:string;languages:string[];downloadPageUrl:string}):{subject:string;html:string;text:string;templateVersion:string}{
  ...
  const languages=input.languages.map(escapeHtml).join(', ')
  const subject=`Your BookLingua translations are ready: ${input.bookTitle}`
  const text=`Hi ${input.authorName},\n\nYour ${input.languages.join(' and ')} translations for ${input.bookTitle} are ready.\n\nDownload your translations: ${input.downloadPageUrl}\n\nThe Final files are the clean versions for editing and upload. The Review file shows BookLingua’s review changes, the Chapter Map matches the original and translated chapters, and the Upload Guide explains the next steps.\n\nHappy publishing,\nThe BookLingua Team`
  return{subject,text,templateVersion:'customer-delivery-v2',html:`<div data-template-version="customer-delivery-v2" style="font-family:Arial,sans-serif;max-width:620px;margin:0 auto;color:#1f2937;line-height:1.6"><div style="padding:24px 0;border-bottom:3px solid #7c3aed"><h1 style="margin:0;color:#5b21b6;font-size:28px">Your translations are ready</h1></div><p>Hi ${escapeHtml(input.authorName||'there')},</p><p>Your <strong>${languages}</strong> translations for <strong>${escapeHtml(input.bookTitle)}</strong> are ready.</p><p style="text-align:center;margin:32px 0"><a href="${escapeHtml(input.downloadPageUrl)}" style="display:inline-block;background:#7c3aed;color:#fff;text-decoration:none;font-weight:700;padding:14px 24px;border-radius:8px">Download your translations</a></p><p>The <strong>Final Translation</strong> files are the clean versions for editing and upload. Use the <strong>Translation Review</strong> to inspect BookLingua’s review changes, and the <strong>Chapter Map</strong> to match each translated chapter to your original manuscript.</p><p>Your <strong>BookLingua Upload Guide</strong> explains how to review, edit, format, and upload your files.</p><p>Happy publishing,<br>The BookLingua Team</p></div>`}
}

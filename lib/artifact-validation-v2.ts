...
function htmlBlocks(xml: string): { headings: string[]; paragraphs: string[]; text: string } {
  const headings = Array.from(xml.matchAll(/<h[1-6]\b[^>]*>([\s\S]*?)<\/h[1-6]>/gi)).map(m => textFromMarkup(m[1])).filter(Boolean)
  const paragraphs = Array.from(xml.matchAll(/<(?:p|li)\b[^>]*>([\s\S]*?)<\/(?:p|li)>/gi)).map(m => textFromMarkup(m[1])).filter(Boolean)
...
export function validateArtifact(buffer: Buffer, kind: ArtifactKind): ArtifactValidationResult {
  ...
  try {
    if (!errors.length) {
      const zip = new AdmZip(buffer); const entries = new Map(zip.getEntries().map(entry => [entry.entryName, entry]))
      if (kind === 'epub') {
        ...
        const rootTag = container?.match(/<rootfile\b[^>]*\/?\s*>/i)?.[0]
        ...
        if (!container || !opfPath || !entries.has(opfPath)) errors.push({ code: 'EPUB_CONTAINER', message: 'EPUB container/OPF is missing or invalid' })
          ...
          for (const tag of opf.match(/<item\b[^>]*\/?\s*>/gi) || []) { const a = attrs(tag); if (a.id && a.href) manifest.set(a.id, { href: a.href, properties: a.properties || '', mediaType: a['media-type'] || '' }) }
          const spine = (opf.match(/<itemref\b[^>]*\/?\s*>/gi) || []).map(tag => attrs(tag).idref).filter(Boolean)
          ...
          for (const id of spine) {
            ...
            if (blocks.headings.length && blocks.paragraphs.join(' ').trim().length < 2) errors.push({ code: 'EMPTY_CHAPTER', message: 'Chapter has no body content', location: entryPath! })
          }
          for (const item of Array.from(manifest.values())) if (/\bnav\b/.test(item.properties)) { const p = safeZipPath(base, item.href); const e = p && entries.get(p); if (e) navigationHeadings.push(...htmlBlocks(e.getData().toString('utf8')).headings) }
          const ncx = Array.from(manifest.values()).find(item => /ncx/i.test(item.mediaType)); if (ncx) { const p = safeZipPath(base, ncx.href); if (!p || !entries.has(p)) errors.push({ code: 'EPUB_NCX', message: 'Declared NCX navigation document is missing' }) }
        }
      } else {
        if (!entries.has('[Content_Types].xml')) errors.push({ code: 'DOCX_CONTENT_TYPES', message: 'DOCX content types are missing' })
        if (!entries.has('_rels/.rels') || !entries.has('word/_rels/document.xml.rels')) errors.push({ code: 'DOCX_RELATIONSHIPS', message: 'DOCX relationships are missing' })
        ...
        if (!document) errors.push({ code: 'DOCX_DOCUMENT_MISSING', message: 'DOCX word/document.xml is missing' })
          ...
          const chapterIndexes = ps.map((p, i) => /^heading1$/i.test(p.style) ? i : -1).filter(i => i >= 0)

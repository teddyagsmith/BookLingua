...
function paragraph(text:string){return new Paragraph({children:[new TextRun(text)]})}
function bullet(text:string){return new Paragraph({children:[new TextRun(text)],bullet:{level:0}})}
...
export async function renderCustomerTranslationNotesDocx(bytes:Buffer,bookTitle:string,language:string):Promise<Buffer>{
  ...
  for(const line of meaningful){
    ...
    if(/^Reason:/i.test(line)){children.push(paragraph(line));continue}

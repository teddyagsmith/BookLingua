...
export default async function CustomerDownloads({params,searchParams}:{params:{orderId:string};searchParams:{token?:string}}){
  ...
  return <main className="min-h-screen bg-gradient-to-b from-violet-50 to-white px-5 py-12 text-gray-900"><div className="mx-auto max-w-4xl"><header className="mb-10"><div className="text-sm font-bold uppercase tracking-widest text-violet-700">BookLingua</div><h1 className="mt-2 text-4xl font-bold">Your translations are ready</h1><p className="mt-3 text-lg text-gray-600">{order.book_title}</p></header><div className="space-y-8">{sections.map(section=><section key={section.language} className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm"><h2 className="text-2xl font-bold">{section.name}</h2><div className="mt-5 grid gap-4 md:grid-cols-2">{section.items.map(item=><a key={item.type} href={item.url} className="rounded-xl border border-violet-100 p-4 transition hover:border-violet-400 hover:bg-violet-50"><div className="font-semibold text-violet-800">{item.label}</div><div className="mt-1 text-sm text-gray-600">{item.description}</div><div className="mt-3 text-xs font-medium text-gray-500">{item.filename}</div></a>)}</div></section>)}<section className="rounded-2xl bg-violet-700 p-6 text-white"><h2 className="text-2xl font-bold">Upload Guide</h2><p className="mt-2 text-violet-100">Review, edit, navigate, and upload your translated files with clear next steps.</p><a href={guideUrl} className="mt-5 inline-block rounded-lg bg-white px-5 py-3 font-semibold text-violet-800">BookLingua Author Upload Guide</a></section></div><p className="mt-10 text-sm text-gray-500">Need help? Email hello@booklingua.io.</p></div></main>
}

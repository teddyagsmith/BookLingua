...
import { verifyCustomerPortalToken, buildCustomerArtifactDownloadUrl,verifyReviewPortalToken,buildReviewArtifactDownloadUrl } from '@/lib/download-token'
...
export default async function CustomerDownloads({params,searchParams}:{params:{orderId:string};searchParams:{token?:string;scope?:string}}){
  const token=searchParams.token||''
  const reviewScope=searchParams.scope==='review'
  if(reviewScope?!verifyReviewPortalToken(params.orderId,token):!verifyCustomerPortalToken(params.orderId,token))return <ErrorCard message="This download link is invalid or incomplete." />
  ...
  if(!order||!allowedStatuses.includes(order.status))return <ErrorCard message={reviewScope?'These files are not ready for internal review.':'These files are not approved for customer delivery.'} />
  ...
  for(const language of languages){
    ...
    const items=customerVisibleArtifacts(order.book_title,row.manifest as PackageManifestV1).map(item=>({...item,url:reviewScope?buildReviewArtifactDownloadUrl(order.id,language,item.type,origin):buildCustomerArtifactDownloadUrl(order.id,language,item.type,origin)}))
    sections.push({language,name:customerLanguageName(language),items})
  }
  const guideUrl=`${origin}/api/download/${order.id}/guide?${reviewScope?'scope=review&':''}token=${encodeURIComponent(token)}`

...
async function run(orderId:string,language:string,format:'epub'|'txt',source:Buffer){const brief=(await db.from('translation_briefs').select('brief').eq('order_id',orderId).eq('language',language).single()).data!.brief;return runSemanticPipeline({supabase:db,orderId,language,sourceFormat:format,source,title:'Moonroot Test',brief,notes:{schemaVersion:'1.0',language,approach:'Synthetic real-model proof.',sections:[{id:'term',title:'Terminology',entries:[{source:'Moonroot',target:'Moonroot',reason:'Author requested exact retention.'}]}]},allowReviewedStructure:true,translate:translator})}

...
for(const language of ['fr','de']){
  ...
  const derived=deriveEditorialTranslationNotes({language:language==='fr'?'French':'German',pass1,pass2,authoritativeTitle:{source:'Bride of the Hollow King',target:titleNode.translatedText},limit:language==='fr'?9:3})
  const broader=[
    ...
    evidence(pass2,/King of the Hollow Court/,`The royal rank and Hollow Court terminology are rendered as one coherent piece of worldbuilding rather than translated word by word in isolation.`),
    evidence(pass2,/Lord of the Blackthorn Wood/,`Blackthorn imagery, rank, and the Thorn Throne are handled consistently within the same fantasy naming system.`),
